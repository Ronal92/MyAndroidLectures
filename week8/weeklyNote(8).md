Weekly Summary!!
		...
		지금까지 프로그래밍한 작업공간은 MainThread였습니다.
		하지만!! 프로그래밍을 하다보면 동시에 2가지 일을 작업해야할 때가 있습니다. 이 때 사용하는 것이 Thread 프로그래밍입니다. 

		쓰레드 프로그래밍은 웹통신에 들어가기 전에 로컬에서 배울수 있는 난이도가 높은 프로그래밍이니 잘 숙지해야 됩니다.^^ 
